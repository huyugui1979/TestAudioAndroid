...
//  Copyright (c) 2014爛 midfar.com. All rights reserved.
//
...
////
    int denoise = 1;
    int noiseSuppress = 5;
    speex_preprocess_ctl(preprocess, SPEEX_PREPROCESS_SET_DENOISE, &denoise); //蔥婑
    speex_preprocess_ctl(preprocess, SPEEX_PREPROCESS_SET_NOISE_SUPPRESS, &noiseSuppress); //扢离婑汒腔dB


    int agc = 1;
    int q=16000;
    //actually default is 8000(0,32768),here make it louder for voice is not loudy enough by default. 8000
    speex_preprocess_ctl(preprocess, SPEEX_PREPROCESS_SET_AGC, &agc);//崝祔
    speex_preprocess_ctl(preprocess, SPEEX_PREPROCESS_SET_AGC_LEVEL,&q);
    int vad = 1;
    int vadProbStart = 80;
    int vadProbContinue = 65;
    speex_preprocess_ctl(preprocess, SPEEX_PREPROCESS_SET_VAD, &vad); //噙秞潰聆
...
SanSpeexEnc::~SanSpeexEnc()
{
    speex_encoder_destroy(enc_state); //每Destroy the encoder state
    speex_echo_state_destroy(echo_state);
    speex_preprocess_state_destroy(preprocess);
    speex_bits_destroy(&enc_bits); //每Destroy the bit-packing struct
//    closeDebugPrint();
}
...
SanSpeexDec::~SanSpeexDec()
{
    ...
    //每Destroy the decoder state
    speex_decoder_destroy(dec_state);

    //每Destroy the bit-stream truct
