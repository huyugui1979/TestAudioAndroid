...
int VoiceSession::SendData(char* data,int len)
{
	...
	if(this->_sendQueue.size()==5)
	{
		...
		LOGD("send seq is %d,crc is %d,size is %d\n",_seq-1,Crc,totalSize+2);
		//
		totalSize+=2;
		//
		unsigned short s = totalSize;
		...
		if(len ==2)
		{
			...
			if(len>0)
			{
				...
				if(m ==1 && s == 100)
				{
					...
					LOGD("recvive a server error,error is %d",v->_serverError);
					v->Signal();
					//
				}
				...
				if(m == 1 && s ==17)
				{
					...
					if(Crc != temp)
					{
						LOGD("crc error,recv crc is %d,seq is %d, len is %d,ll is %d\n",temp,*seq,len,ll);

						continue;
					}

					LOGD("recv seq is %d,crc is%d,size is %d\n",*seq,Crc,len);
				...
				if(m==1 && s ==16)//room list
				{
					...
					char* t = (char*)(udpBuffer+2);
					unsigned short count = *(unsigned short*)t;
					t=t+2;
					for(int i =0;i<count;i++)
					{
				...
				if(m==1 && s==22) //member join echo
				{
					...
					v->_serverEvent(22,c);
					LOGD("playerId %d get in",c->player_id);
					//
				...
				if(m==1 && s==24) //member left echo
				{
					...
					delete c;
					//
					LOGD("playerId %d get out",c->player_id);
...
int VoiceSession::SendDataToServer(char* buffer, int len)
{
	...
	if(this->Wait(3)!= -1)
	{
		...
		{
			LOGD("recve server error,error is %d",this->_serverError);
...
int VoiceSession::InitSockStruct(const char* address,short port)
{
	...
	if (-1 == _sockfd) {
		...
		LOGD("Failed to create socket,%d",errno);
		return CONNECT_FAILED;
	}
	int  set = 1;
	setsockopt(_sockfd, SOL_SOCKET, MSG_NOSIGNAL, (void  *)&set, sizeof(int));
